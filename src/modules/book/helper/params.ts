...
import { IBookParams } from "../interface/params.interface";
export class BookParams {
	getParams(
		...
		if (Number.isNaN(bookId))
			response.error(res, "bookId number is not a number", 400, "json");
		params.bookId = bookId;
		if (urlSplited.length <= 4) return params;
		const pageId = Number(urlSplited[4] ?? "NaN");
		if (Number.isNaN(pageId))
			response.error(res, "pageId number is not a number", 400, "json");
